...
class SpiderKomp(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        loader = ItemLoader(item=ProductPhoneItem(), response=response)
        loader.add_css("name", "section#p-inner-name h1::text")
        loader.add_value('price', json_data.get("offers", {}).get("price", "notfound"))
        loader.add_value('vendor', json_data.get("brand", {}).get("name", "notfound"))
